class BoardsController < ApplicationController
  ...
  def add_post
    ...
    the_post.board_id = params[:board_id]
    the_board = Board.find(the_post.board_id)

    if the_post.valid?
      the_post.save
      redirect_to("/boards/#{the_post.board_id}", { :notice => "Board updated successfully."} )
    else
      redirect_to("/boards/#{the_post.board_id}", { :alert => the_board.errors.full_messages.to_sentence })
    end
  end
